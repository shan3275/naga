...
berr domain_domain_default_act_set(naga_acl_t *acl)
{
	memcpy(&domain_default_acl, acl, sizeof(naga_acl_t));
	return E_SUCCESS;
}

berr domain_domain_default_act_get(naga_acl_t *acl)
{
	memcpy(acl, &domain_default_acl, sizeof(naga_acl_t));
	return E_SUCCESS;
}
